...
<script>
  ...
  function calculerIndicateurs() {
    ...
    var resultatHTML = '<p class="resultat">Résultat: ' + resultat.toFixed(2) + ' Fcfa</p>' +
      ...
      '<p>Interprétation:</p>' +
      '<p>Le chiffre d\'affaires qui vous permet de faire ni bénéfice ni perte est de: <span class="resultat">.' + seuilDeRentabilite.toFixed(2) + ' Fcfa</span> Cela traduit votre seuil de rentabilité. Avec ce seuil,vous ne pouvez pas dégager des bénéfices mais aussi vous ne subissez pas de pertes ce qui signifie que votre résultat sera égal à zéro(0 Fcfa).</p>' + 
